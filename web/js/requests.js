...
document.addEventListener("DOMContentLoaded", () => {
  const pointMMSIInput = document.getElementById("point-mmsi");
  const filterMMSIInput = document.getElementById("filter-mmsi");

    // Management of auto completion for MMSI input -> just put all the MMSI values in the input while typing
  pointMMSIInput.addEventListener("input", () => {
    const mmsi = pointMMSIInput.value.trim();
    if (mmsi.length >= 3) {
      fetch(`php/requests.php/get_all_mmsi`)
        .then((response) => response.json())
        .then((data) => {
          const filteredMMSI = data.filter((item) =>
            item.startsWith(mmsi)
          );
          pointMMSIInput.setAttribute(
            "list",
            "mmsi-list"
          ); // Set the datalist attribute
          const dataList = document.getElementById("mmsi-list");
          dataList.innerHTML = ""; // Clear previous options
          filteredMMSI.forEach((item) => {
      ...
      pointMMSIInput.removeAttribute("list");
      const dataList = document.getElementById("mmsi-list");
      dataList.innerHTML = ""; // Clear previous options
    }
  });
...
//   // Management of auto completion for MMSI input -> just put all the MMSI values in the input while typing
//   filterMMSIInput.addEventListener("input", () => {
//     const mmsi = filterMMSIInput.value.trim();
//     if (mmsi.length >= 3) {
//       fetch(`php/requests.php/get_all_mmsi`)
//         .then((response) => response.json())
//         .then((data) => {
...
//         .catch((error) => console.error("Erreur:", error));
//     } else {
//       filterMMSIInput.removeAttribute("list");  
//       const dataList = document.getElementById("filter-mmsi-list");
//       dataList.innerHTML = ""; // Clear previous options
//     }
//   });
